...
function call_from_db(params) {
    ...
    $.ajax({
        type: "GET",
        url: "api/history.php?p=" + params + "&q=1",
    ...
    if (!inc) {
        ...
        function moveScroll() {
            ...
            canvas.addEventListener('click', (event) => {
                ...
                // console.log(x + '-' + y);
                if (x >= left - 15 && x <= left + 15 && y >= down && y <= up) {
                    myChart.options.scales.x.min = myChart.options.scales.x.min - 24;
                    myChart.options.scales.x.max = myChart.options.scales.x.max - 24;
                    if (myChart.options.scales.x.min <= 0) {
                        myChart.options.scales.x.min = 0;
                        myChart.options.scales.x.max = 70;
                    }
                }
                ...
                if (x >= right - 15 && x <= right + 15 && y >= down && y <= up) {
                    ...
                    if (myChart.options.scales.x.max >= data.datasets[0].data.length) {
                        myChart.options.scales.x.min = data.datasets[0].data.length - 70;
